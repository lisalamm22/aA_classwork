...
function flatten(data) {
    ...
    let flattened = [];
    data.forEach(ele => { Array.isArray(ele) ? flattened=flattened.concat(flatten(ele)) : flattened.push(ele)})
    return flattened
}
...
function fileFinder(directories, targetFile) {
    childrenDirs = Object.keys(directories)
    let found = false
    childrenDirs.forEach( dir => {
        if(dir[0] === "/"){
            if(fileFinder(directories[dir], targetFile)){
                found = true
            }
        }
        else if(dir === targetFile){
            found = true
        }
    })
    return found
...
function pathFinder(directories, targetFile) {
    ...
    childrenDirs.forEach( dir => {
        if(dir[0] === "/"){
            if(pathFinder(directories[dir], targetFile)){
                path += dir + pathFinder(directories[dir], targetFile)
